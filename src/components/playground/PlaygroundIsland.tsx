...
export default function PlaygroundIsland({
  ...
  const [navPos, setNavPos] = useState<{top:number,left:number}>({top:0,left:0});
  ...
  const applyModeToIframe = (m: Mode) => {
    ...
    try {
      ifr?.contentWindow?.postMessage({ type: 'oxbow-set-dark', enable: enableDark }, '*');
      // Recompute height after theme changes
      setTimeout(() => ifr?.contentWindow?.postMessage({ type: 'oxbow-request-height' }, '*'), 20);
      setTimeout(() => ifr?.contentWindow?.postMessage({ type: 'oxbow-request-height' }, '*'), 120);
    } catch {}
  };
  ...
  useEffect(() => {
    const onEsc = (e:KeyboardEvent)=>{ if(e.key==='Escape') setNavOpen(null); };
    const onClick = (e:MouseEvent)=>{ const t = e.target as Node; if(navMenuRef.current?.contains(t)) return; setNavOpen(null); };
        ...
        <div className="items-center justify-end hidden  md:flex gap-2">
          {arguments[0]?.subsByCat && (
            <>
              ...
              {navOpen==='sub' && (
                <div ref={navMenuRef} className="fixed z-50 mt-2 w-64 rounded-xl outline outline-zinc-100 shadow bg-white text-xs text-zinc-600 divide-y divide-zinc-100" style={{top:navPos.top,left:navPos.left}}>
                  <div className="py-2 max-h-64 overflow-auto">
        ...
        {canSeeCode && tab === 'code' && (
          <div className="code-pane flex-grow p-4 text-xs bg-white size-full selection:bg-zinc-100 scrollbar-hide">
            {mode==='system' && !!arguments[0]?.highlightedSystem ? (
              <div className="overflow-x-auto scrollbar-hide" dangerouslySetInnerHTML={{ __html: arguments[0]!.highlightedSystem! }} />
            ) : mode==='light' && !!arguments[0]?.highlightedLight ? (
              <div className="overflow-x-auto scrollbar-hide" dangerouslySetInnerHTML={{ __html: arguments[0]!.highlightedLight! }} />
            ) : mode==='dark' && !!arguments[0]?.highlightedDark ? (
              <div className="overflow-x-auto scrollbar-hide" dangerouslySetInnerHTML={{ __html: arguments[0]!.highlightedDark! }} />
            ) : (
              <pre className="whitespace-pre overflow-x-auto scrollbar-hide"><code>{codeText}</code></pre>
            )}
          </div>
        )}
      </div>
