...
export default function DetailToolbarIsland({ cat, sub, idx, subsByCat, counts, prevHref, nextHref }: Props){
  ...
  const CatMenu = () => (
    <div ref={menuRef} className="fixed z-50 w-56 mt-2 text-xs bg-white shadow rounded-xl outline outline-base-100 text-base-600 divide-y divide-base-100" style={{top:menuPos.top, left:menuPos.left}}>
  ...
  const SubMenu = () => (
    <div ref={menuRef} className="fixed z-50 w-64 mt-2 text-xs bg-white shadow rounded-xl outline outline-base-100 text-base-600 divide-y divide-base-100" style={{top:menuPos.top, left:menuPos.left}}>
      <div className="py-2 overflow-auto max-h-64">
  ...
  const IdxMenu = () => (
    <div ref={menuRef} className="fixed z-50 w-40 mt-2 text-xs bg-white shadow rounded-xl outline outline-base-100 text-base-600" style={{top:menuPos.top, left:menuPos.left}}>
  ...
  return (
    <div className="flex items-center gap-2">
      {/* Category */}
      <button ref={btnRef} onClick={(e)=>openMenu('cat', e)} className="inline-flex items-center gap-2 h-[28px] px-2 py-1 rounded-lg bg-white outline outline-1 outline-base-200 text-base-700 text-xs">
      ...
      {/* Block */}
      <button onClick={(e)=>openMenu('sub', e)} className="hidden md:inline-flex items-center gap-2 h-[28px] px-2 py-1 rounded-lg bg-white outline outline-1 outline-base-200 text-base-700 text-xs">
      ...
      {/* Number */}
      <button onClick={(e)=>openMenu('idx', e)} className="hidden md:inline-flex items-center  h-[28px] px-2 py-1 rounded-lg bg-white outline outline-1 outline-base-200 text-base-700 text-xs">
        <span>#0</span>
      ...
      {/* Divider */}
      <div aria-hidden className="mx-1 h-4 w-[1px] bg-base-200 hidden md:inline-block"/>

      {/* Pagination */}
      <nav className="relative z-0 inline-flex gap-1.5" aria-label="Pagination">
        {prevHref ? (
          <a href={prevHref} aria-label="Go to previous page" className="inline-flex items-center justify-center h-7 w-7 rounded-md outline outline-1 outline-base-200"><ChevronLeft className="size-4"/></a>
        ) : (
          <div className="inline-flex items-center justify-center opacity-50 h-7 w-7 rounded-md outline outline-1 outline-base-100"><ChevronLeft className="size-4"/></div>
        )}
        {nextHref ? (
          <a href={nextHref} aria-label="Go to next page" className="inline-flex items-center justify-center h-7 w-7 rounded-md outline outline-1 outline-base-200"><ChevronRight className="size-4"/></a>
        ) : (
          <div className="inline-flex items-center justify-center opacity-50 h-7 w-7 rounded-md outline outline-1 outline-base-100"><ChevronRight className="size-4"/></div>
        )}
      </nav>
    </div>
  );
