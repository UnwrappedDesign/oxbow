...
page: Pricing
---
import PaymentBrands from "@/components/global/PaymentBrands.astro";
...
When you purchase an all-access license, you unlock everything: every component example and all future additions to Oxbow UI Pro.
...
- **Unlimited projects** — Use Oxbow UI for as many projects as you need.
...
## <a href="/api/buy/individual" type="submit" class="inline-flex items-center justify-between w-full gap-4 px-4 py-3 h-10 text-sm font-medium text-white duration-300 transition rounded-lg bg-accent-500 group active:transition-none outline-offset-2 hover:bg-accent-600   ring-4 ring-blue-50 not-prose " aria-label="Get your link"> Get all access for <span> $149</span></a>
...
Access all components, elements and layouts, expertly crafted and ready to be use in your Tailwind projects.
...
Vanilla HTML, Tailwind CSS & Alpine JS — Interactive component include a functional example in HTML, Tailwind CSS with Alpine JS included where necessary.
...
- **Free updates and content** — New tcomponents, elements and layouts, and versions are included at no extra charge.
